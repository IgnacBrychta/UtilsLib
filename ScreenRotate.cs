...
internal class DisplayRotation
{
	internal static readonly TimeSpan rotationDelay = new TimeSpan(0, 0, 5);
	...
	internal static bool Rotate(uint DisplayNumber, Orientations Orientation)
	{
		if (DisplayNumber == 0)
	...
	internal static void ResetAllRotations()
	{
		try
		{
			uint i = 0;
	...
	internal static void RotateAllScreens(uint amount)
	{
		int monitors = MonitorInfo.GetDisplayCount();
		for (int step = 0; step < amount; step++)
		{
			for (int i = 1; i < 5; i++)
			{
				for (uint j = 1; j <= monitors; j++)
				{
					Rotate(j, (Orientations)(i % 4));
				}
